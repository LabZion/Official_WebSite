        ...
        data: [
            {
                ...
                content:
                    ...
                    '\n' +
                    '\n' +
                    '随着国际业务的发展，与不同地区车主的增加，管理不同区域车主数据的同时，满足不同地区数据管理法律法规的要求成为了企业乃至其他国际车企在数字化战略方向的一个挑战。2017年实行的《中华人民共和国网络安全法》第37条规定： “关键信息基础设施的运营者在中华人民共和国境内运营中收集和产生的个人信息和重要数据应当在境内存储。因业务需要，确需向境外提供的，应当按照国家网信部门会同国务院有关部门制定的办法进行安全评估；法律、行政法规另有规定的，依照其规定。”也就是说，在中国境内运营时收集的个人信息应在中国境内存储。我们需要基于AWS平台，将数据和DevOps架构迁移到中国。在保护隐私数据的前提下，将车主信息从中国同步回车企所在国，用于商业分析。因为团队成员组成复杂，来自不同国家和地区。客户stakeholder众多，仅PO就有三个，因此项目十分复杂。\n' +
                    '\n' +
                    '### 解决方案\n' +
                    '\n' +
                    '\n' +
                    ...
                    '\n' +
                    '\n' +
                    'ThoughtWorks安全团队在对客户的数据进行迁移的过程中克服了LDAP与DynamoDB异构数据库的切换、Serverless下的mTLS实现、Serverless下的JMS异步消费、数据同步中的Exactly-once保证等问题，为不同地区与国家的数据合规、跨区域数据的透明迁移、AWS Global与CN的数据异地备份等问题积累了丰富的经验，在不影响客户现有业务的同时，在严格符合相关国家法律的基础上，完美实现了数据迁移的工作。'
            },
            {
                customer: '安全流水线升级案例',
                ...
                content:'\n' +
                    ...
                    '\n' +
                    '\n' +
                    'ThoughtWorks安全团队为客户打造一个从框架设计，到落地，到工具化的DevSecOps整体解决方案，将IT安全防护融入整个生命周期中，助力企业保证交付质量，提升开发速度。'
            },
            {
                ...
                content:
                    ...
                    '\n' +
                    '\n' +
                    '当前车载智能终端（车机）应用日益广泛，智能汽车已是大势所趋。在整个汽车生态体系中，车机由不同的供应商提供，每个车机又由很多的组件组装而成。在这个由多个供应商组成的生态系统中，对安全漏洞的追踪和确认变得非常困难。而且车机厂商大多关注于产品功能，忽视产品安全。而安全威胁会给智能汽车带来严重后果，甚至损害整车厂商品牌。对于车厂而言，往往在最终拿到的只是一堆的进制文件，那如何在没有源码的前提下，发现其中存在的漏洞，目前业界的做法都是采用人工逆向分析技术，去发现二进制文件中的漏洞。但是存在的问题是，当设备很多的时候，人工分析太慢，产品迭代太慢，还可能出现误判。企业急需打破安全监测供需瓶颈。项目挑战在于涉及内核和底层界别修改，技术挑战大；在没有源代码的情况下，要求团队具有逆向工程能力；有源码与无源码需要不同的解决方案。\n' +
                    '\n' +
                    '### 解决方案\n' +
                    '\n' +
                    '\n' +
                    '\n' +
                    'ThoughtWorks安全团队通过和国内多家大型车企以及智能设备生成厂商的多年合作，利用积累的经验为客户提供了一套对二进制自动化漏洞扫描以及加固的解决方案。解决方案有2个亮点：\n' +
                    '\n' +
                    '##### 1、二进制漏洞自动化扫描\n' +
                    '\n' +
                    ...
                    '\n' +
                    '\n' +
                    'ThoughtWorks安全团队对二进制自动化漏洞扫描以及加固的解决方案解决了传统的二进制漏洞检测难度大，检测周期长的问题。同时研发了一套漏洞自动化修复框架，在准确和快速地检测出二进制文件中漏洞的同时，还可以自动化修复漏洞，为业界的智能设备产品公司在快速迭代的过程中保驾护航。'
            },
        ]
    },
    cloud: {
        ...
        data: [
            {
                ...
                content:
                    ...
                    '\n' +
                    '###解决方案\n' +
                    '>Thoughtorks安全B团队面向除了客户企业员工以外的合作伙伴、供应商、客户和消费者提供移动办公套件身份认证服务。服务包括标准的SAML/OAuth等Web、移动应用认证和授权协议，提供基于多因子和风险控制分析的强身份认证，集成社交账号和企业账号联邦认证，并实现多租户管理等。核心价值包括以下几点：\n' +
                    '#####1、\tSSO单点登录\n' +
                    ...
                    '###客户价值\n' +
                    '>Thoughtorks安全B团队开发的身份认证及权限管理平台，拥有安全性高、扩展能力良好的特点，针对企业内部拥有多个子系统，员工使用各个系统时需要使用不同的用户信息登录，企业期望实现单点登录，统一认证入口的需求，以及企业需要细粒度管控员工对资源和服务的访问权限的需求，但不局限于云平台和公司规模，帮助企业实现高效、统一管理身份信息和权限信息，助力企业提高生产效率，适合于政府、运营商、企业、金融、军队、医疗、教育、制造业等大型企事业单位。\n' +
                    ' \n' +
                    '\n'
            },
            {
                ...
                content:
                    ...
                    '\n' +
                    '\n' +
                    'Thoughtorks安全B团队研发的智能信息资产管控软件改进了客户的旧版本软件只能监控和审计特定软件，无法达到系统级别的监控的问题。在多PC平台上实现权限控制功能的同时为员工提供了顺畅的系统使用体验，成功让审计流程在系统中透明化、无感化。适用于企业内部大规模使用PC端工作，重视资源的安全与保护，企业对不同角色员工的访问权限有定制化需求，以及企业想要自定义内部资源类型，比如定义哪些属于敏感信息等场景。助力企业大幅度性能提升，为员工提供无障碍体验。'
            },
            {
                ...
                content:
                    ...
                    '\n' +
                    '\n' +
                    'ThoughtWorks安全团队经历5个月时间的努力，将客户企业内部的VPN连接从2个，扩展到同时支持40个，适用平台包括Windows7、Windows10，大大提升了员工的沟通效率和工作体验。'
            },
            {
                customer: '源码可信检验实施案例',
                ...
                content:
                    ...
                    '\n' +
                    '\n' +
                    '源码可信意思是指验证软件整个开发过程是否符合企业的内部规范，安全可信。验证过程包含编码规范、编译、构建、部署与发布。对于源码要进行代码规范验证，对于编辑构建要进行第三方依赖和一键部署验证，对于部署发布要进行一致性验证。我们的客户出口欧洲的产品都要获得CSEC（英国通信电子安全小组）的证书，确保产品符合英国政府的安全标准。\n' +
                    '\n' +
                    '### 解决方案\n' +
                    '\n' +
                    '\n' +
                    ...
                    '\n' +
                    '\n' +
                    'Thoughtorks安全团队提供的四步白盒验证方案帮助企业建立了统一的检验标准和流程，保证产品在CESG检查之前先在内部实现风险管控，在源头处对产品合规进行把控，减小企业风险与损失。'
            },
        ...
        data: [
            {
                ...
                content:
                    ...
                    '\n' +
                    '\n' +
                    '近年以来，客户企业的基础设施与业务支撑系统受到了大量的来自互联网端的威胁，造成了一定的经济损失，2019年9月，由于遭受到 Sin Eater 病毒的攻击，客户企业的基础设施与系统发生了中断，直接威胁到了企业的正常运营，造成了经济损失与财务损失。2019年9月下旬，ThoughtWorks安全团队在接受到企业的邀请后，在现场进行了一些调查，根据这些调查与现场访谈的结果，我们认识到需要对网络基础设施到应用层与数据层都安全需要进行一次完整的评估，并设计与实施相应安全加固方案。\n' +
                    '\n' +
                    '### 解决方案\n' +
                    '\n' +
                    '\n' +
                    ...
                    '\n' +
                    '\n' +
                    '- 使用了专门的存储服务器，定期对数据做备份，即便服务器的数据被加密，也能很快地恢复数据。\n' +
                    '\n' +
                    '- 使用VPN防火墙替换原来的防火墙，保障运维安全。原始的Windows远程桌面存在很多安全漏洞，比如永恒之蓝。使用VPN的方式，用户不能直接访问服务器，需要通过VPN拨号才能访问。\n' +
                    '\n' +
                    ...
                    '\n' +
                    '\n' +
                    'ThoughtWorks安全团队不仅为客户量身定制了基础设施安全治理方案，同时提供不定期的安全咨询和设备升级计划，为企业的信息安全保驾护航。'
            },
