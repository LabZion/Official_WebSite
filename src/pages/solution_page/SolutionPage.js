...
const industry = {
    ...
    data: {
        ...
        data: [
            ...
            description: 'GDPR是在欧盟法律中对所有欧盟个人关于数据保护和隐私的规范，涉及了欧洲境外的个人数据出口。GDPR 主要目标为取回个人对于个人数据的控制，以及为了国际商务而简化在欧盟内的统一规范。GDPR的规范要求企业从整个公司乃至具体业务系统和产品都能够遵守一定的规范，其业务包含数据治理、数据泄露保护、数据出境的具体方面。ThoughtWorks安全团队将隐私保护作为一个单独的安全方向，结合不同国家地区的隐私保护法律法规，通过保护数据机密性、数据脱敏等方法，结合自动化等手段，提供一套高效可落地的隐私保护解决方案。该解决方案主要面对如车企、金融、保险、零售、服务等国际化企业，提供数据在不同区域的保护与隐私合规。\n'
        },{
                title: '可搜索加密',
                img: 'https://raw.githubusercontent.com/LabZion/Official_WebSite/master/public/picture/data2.jpg',
                description: '可搜索加密是一种密文检索的解决方案，其主要解决的问题是在数据高机密的场景下，如何保证数据的正常检索使用。可搜索加密与同态加密类似，都是在数据加密后的信息上进行操作，使得数据本身的信息在操作中不会泄露。可搜索加密基于底层密码学的应用，在加密数据的同时，生成同样加密的索引键，通过索引键的特殊构造达到数据的搜索。其一般的应用场景是在高机密的数据安全要求下，在数据存储是被字段级加密的情况下，同时需要保持一定的数据可用性。ThoughtWorks安全团队针对澳洲支付平台的政府支付项目，交付了基于可搜索加密的解决方案，帮助客户在高机密的场景下保证数据可用性。'
            },{
                title: '安全多方计算',
    ...
    embed: {
        title: '嵌入式系统与物联网',
        img:"url('https://raw.githubusercontent.com/LabZion/Official_WebSite/master/public/picture/image.png')",
